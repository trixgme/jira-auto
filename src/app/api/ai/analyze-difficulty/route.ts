    ...
    try {
      ...
      const validatedResult = {
        ...
        reasoning: language === 'en' 
          ? (result.reasoning || "Unable to complete analysis.")
          : (result.reasoningKo || result.reasoning || "분석을 완료할 수 없습니다."),
      ...
      if (issueKey) {
        try {
          ...
          const commentText = `🤖 AI Difficulty Analysis Result

Difficulty: ${'⭐'.repeat(validatedResult.difficulty)} (${validatedResult.difficulty}/5)
Estimated Time: ${validatedResult.estimatedHours} hours

📊 Analysis Reasoning:
${result.reasoning || "Unable to complete analysis."}

_This comment was automatically generated by AI._`;
      ...
      const fallbackResult = {
        difficulty: 3,
        reasoning: language === 'en' 
          ? "Failed to parse AI response. Please review the issue manually."
          : "AI 응답 파싱에 실패했습니다. 이슈를 수동으로 검토해주세요.",
        estimatedHours: 8,
        commentAdded: false
      };
